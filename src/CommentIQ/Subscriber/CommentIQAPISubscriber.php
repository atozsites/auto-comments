...
class CommentIQ_Subscriber_CommentIQAPISubscriber implements CommentIQ_EventManagement_SubscriberInterface
{
    ...
    /**
     * Compares a potential atozsites comment to another WordPress comment.
     *
     * To be considered, a comment must be shorter than 100 characters. If the given
     * comment doesn't match this criteria. The atozsites comment is returned. Otherwise, the
     ...
     *
     * @param mixed      $atozsites_comment
     * @param WP_Comment $comment
     *
     * @return mixed
     */
    private function compare_comments($atozsites_comment, WP_Comment $comment)
    {
        if (!$this->is_valid_comment($comment)) {
            return $atozsites_comment;
        }
        ...
        $atozsites_comment_details = get_comment_meta($atozsites_comment->comment_ID, $this->comment_details_meta_key, true);

        if ($comment_details['ArticleRelevance'] > $atozsites_comment_details['ArticleRelevance']) {
            $atozsites_comment = $comment;
        }
    ...
    /**
     * Update the atozsites comment for the given post.
     *
     * @param WP_Post $post
     */
