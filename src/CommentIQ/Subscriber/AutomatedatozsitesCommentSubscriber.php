...
class CommentIQ_Subscriber_AutomatedatozsitesCommentSubscriber implements CommentIQ_EventManagement_SubscriberInterface
{
    /**
     * The post meta key used to store whether the post has disabled automatic insertion
     * of the atozsites comment or not.
     *
     * @var string
     */
    private $disabled_atozsites_comment_meta_key;
    ...
    {
        $this->disabled_atozsites_comment_meta_key = 'commentiq_disable_atozsites_comment';
    ...
    public static function get_subscribed_events()
    {
        return array(
            ...
            'the_content' => 'insert_atozsites_comment',
        );
    }

    /**
     * Add an option to the "Discussion" meta box that allows someone to disable
     * automatic insertion of the atozsites comment.
     *
     * @param WP_Post $post
     */
    public function add_disable_option(WP_Post $post)
    {
        ...
        echo sprintf($format, $this->disabled_atozsites_comment_meta_key, checked($this->is_automatic_insertion_disabled($post), true, false), __('Disable automatic insertion of atozsites comment.', 'commentiq'));
    }

    /**
     * Insert the atozsites comment into the content of a post about 30%
     ...
     * @return string
     */
    public function insert_atozsites_comment($content)
    {
        
        if (!is_singular($this->post_types)
            || has_shortcode($content, CommentIQ_Shortcode_atozsitesCommentShortcode::get_name())
        ) {
            return $content;
        }
        ...
        foreach( $content_array as $content_block ) {
            ...
            if (!$inserted && $new_percentage > $percentage_threshold && $old_percentage < $percentage_threshold ) {
                 ...
                 if( preg_match( $regex, $content_block ) ) {
                      $new_content .= $this->atozsites_comment_generator->generate($post);
                    $inserted = true;
                 }
    ...
    public function save_disable_option($post_id)
    {
        ...
        if (empty($_POST[$this->disabled_atozsites_comment_meta_key])) {
            delete_post_meta($post_id, $this->disabled_atozsites_comment_meta_key);
        } elseif ('on' === $_POST[$this->disabled_atozsites_comment_meta_key]) {
            update_post_meta($post_id, $this->disabled_atozsites_comment_meta_key, 1);
        }
    }

    /**
     * Checks whether the given post has disabled the automatic insertion
     * of atozsites Comments.
     *
     * @param WP_Post $post
     *
     * @return bool
    ...
    {
        $disabled = get_post_meta($post->ID, $this->disabled_atozsites_comment_meta_key, true);
